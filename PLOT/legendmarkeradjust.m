...
%         set(l1,'location','south')
    ...
    switch length(varargin)
        case 1
            marksize=varargin{1};
            for m=1:length(s2)
                set(s2(m),'markersize',marksize,'Oreantation','horizontal');
            end
        case 2
            marksize=varargin{1};
            lwidth=varargin{2};
            for m=1:length(s2)
                set(s2(m),'markersize',marksize,'linewidth',lwidth,'Orientation','horizontal');
            end
    end
   end