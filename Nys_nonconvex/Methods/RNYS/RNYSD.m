function [w, infos] = RNYSD(problem, in_options,reg,co,ct,set,gamma)
    ...
    while (epoch < options.max_epoch)
            w0 = w;
                    ...
                    if gamma == 0.5            
                        ...
                        rho = max(co*min(rho_old,sqrt(g)),1e-4) + ct*max(0,-min(eg));
                        rho_old = sqrt(g);

                    elseif gamma ==1

                        rho = max(co*min(rho_old,(g)),1e-4) + ct*max(0,-min(eg));
                        rho_old = (g);
