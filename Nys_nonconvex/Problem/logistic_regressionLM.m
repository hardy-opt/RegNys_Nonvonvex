classdef logistic_regressionLM
    ...
    methods
        ...
        function obj = logistic_regressionLM(x_train,y_train,x_test,y_test,varargin)
            obj.x_train = x_train;
            obj.y_train = y_train;
            obj.x_test = x_test;
            obj.y_test = y_test;
...
%          
         ...
         function f = cost_batch(obj, w, indices)
            ...
            f = -sum(log(sigmoid(obj.y_train(indices).*(w'*obj.x_train(:,indices)))),2)/obj.n_train + obj.lambda * (w'*w) / 2;
       ...
       % end
        ...
        function h = hess(obj, w, indices)
         ...
         function h1 = hess_sqrt(obj, w, indices)
            ...
            h1 = (1/sqrt(length(indices)))*(obj.x_train(:,indices) .* (obj.y_train(indices).^2 .* sqrt(c)));
...
%             
        ...
        %Computation of C and M for randomized subspace Newton
         ...
         function [M] =  randomized_sub(obj,w,indices,set)
            dime = length(w);
            l = length(set);
            S = eye(dime);
            S = S(:,set);
           ...
           % fprintf('Approximate Hessian Started\n');
            ...
            h1 = S'*(obj.x_train(:,indices) .* (obj.y_train(indices).^2 .* sqrt(c)));
