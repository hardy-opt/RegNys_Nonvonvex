classdef logistic_regression1N
    ...
    methods
        ...
        function obj = logistic_regression1N(x_train,y_train,x_test,y_test,varargin)
            obj.x_train = x_train;
            obj.y_train = y_train;
            obj.x_test = x_test;
            obj.y_test = y_test;
        ...
        function f = cost(obj,w)
            ...
            f = -sum(log(sigmod_result),2) + obj.lambda * (w'*w) / 2;
%          
        end
      ...
      %  function g = sgrad(obj,w,ind)  %%% Stochastic gradient for "one" single index
       ...
       % end
        ...
        function g = grad(obj,w,ind) %%% sum of stochastic gradient to get full gradient
            ...
            g = -sum(((ones(length(ind),1)-sigmoid(xy'*w)).*xy')',2)+obj.lambda*w;
        ...
        function h1 = hess_sqrt(obj, w, indices)
            ...
            h1 = (obj.x_train(:,indices) .* (obj.y_train(indices).^2 .* sqrt(c)));
        end
        function h = hess(obj, w, indices)
            ...
            h = h1 * obj.x_train(:,indices)';%+obj.lambda*eye(obj.d);
        end
