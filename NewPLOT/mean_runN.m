function [mu, sg] = mean_runN(dataset, method, lambda, eta, c_two, EPOCHS, run, COL, BS,c_one)
    i=0;
    ratio=0.1;
    K = containers.Map();
    ...
    for j = 1:run
        if c_two > -1
            if COL>0
                ...
                Name = strcat(dataset, sprintf('K%d_B%d_%s_%.1e_reg_%.1e_ctwo_%.1e_cone_%.1e_run_%d.mat', COL, BS, method, eta, lambda, c_two, c_one,j));
                %Name = strcat(dataset, sprintf('K%d_B%d_%s_%.1e_R_%.1e_c_two_%.1e_run_%d.mat', COL, BS, method, eta, lambda, c_two, j));
            else
                Name = strcat(dataset, sprintf('K%d_B%d_%s_%.1e_reg_%.1e_ctwo_%.1e_run_%d.mat', K(dataset), BS, method, eta, lambda, c_two, j));
            end
        else
            Name = strcat(dataset, sprintf('B%d_%s_%.1e_reg_%.1e_run_%d.mat', BS, method, eta, lambda, j));
        end
%         disp(Name);
        if exist(Name, 'file')
            d = load(Name);
