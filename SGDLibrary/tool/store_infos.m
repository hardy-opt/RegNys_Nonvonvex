...
% Modified by H.Kasai on Oct. 30, 2020
    ...
    if ~epoch
        ...
        infos.val_cost = problem.test_cost(w);
        
        
        p_tr = problem.prediction(w,'Tr');
        infos.acc_tr = problem.accuracy(p_tr,'Tr');
        
        
        
        p_vl = problem.prediction(w,'Vl');
        infos.acc_val = problem.accuracy(p_vl,'Vl');
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%
        valcost = problem.test_cost(w); %%%val cost
        infos.val_cost = [infos.val_cost valcost];
        %%%%
        p_tr = problem.prediction(w,'Tr'); %%% Train Accuracy
        acctr = problem.accuracy(p_tr,'Tr');
        infos.acc_tr = [infos.acc_tr acctr]; 
        %%%
        p_vl = problem.prediction(w,'Vl');  %%% Val/Test accuracy
        accval = problem.accuracy(p_vl,'Vl');
        infos.acc_val = [infos.acc_val accval]; 
        %%%%%%%%%%%%%%%%%%%%%%%%%
