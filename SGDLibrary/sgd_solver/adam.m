    ...
    while  (epoch < options.max_epoch)
        ...
        for j = 1 : num_of_bachces
            ...
            if strcmp(options.sub_mode, 'Adam')
                ...
                % update w
                w = w - steps * m_hat ./ (sqrt(v_hat) + options.epsilon);                
                
                gn =  m_hat ./ (sqrt(v_hat) + options.epsilon);                
            ...
            if any(isnan(grad)) || any(isinf(grad)) || any(isnan(w)) || any(isinf(w))
                    return;
            end
        ...
        % display infos
        if options.verbose > 0
            fprintf('Adam-%s: Epoch = %03d, cost = %.16e, optgap = %.4e, |g| = %.4e\n', options.sub_mode, epoch, f_val, optgap,g);
        end
    end
